...
One caution, the ~herbie totag~ command is rather slow, likely because
it issues multiple calls to ~herbstclient~.  It runs in about 200ms on
my laptop which is enough to produce noticeable lag.  The same
behavior can be achieved about 10x faster by having ~herbstclient~
perform the logic.  This can be done for example:
...
hc keybind "$Mod-$key" substitute PRE tags.focus.index \
  chain + use_index "$i" + or , compare tags.focus.index != PRE , use_previous 
#+end_example
